    ...
    <div style={{ marginTop:"32px", borderTop:`1px solid ${dark?"rgba(255,255,255,0.06)":"#e2e8f0"}`, paddingTop:"24px" }}>
      {status === 'done' ? (
        ...
        <button onClick={() => setOpen(true)} style={{
          background:"transparent", cursor:"pointer", fontSize:"13px", padding:"10px 20px", borderRadius:"8px",
          border: dark ? "1px solid rgba(255,255,255,0.1)" : "1px solid #cbd5e1",
          color: dark ? "#789" : "#64748b",
        }}>⚑ {t.btn}</button>
      ) : (
        <div style={{ borderRadius:"12px", padding:"24px", background: dark ? "rgba(255,255,255,0.02)" : "#f8fafc", border: dark ? "1px solid rgba(255,255,255,0.08)" : "1px solid #e2e8f0" }}>
          ...
          <div style={{ display:"flex", gap:"8px", marginTop:"16px" }}>
            ...
            <button onClick={() => setOpen(false)} style={{
              background:"transparent", padding:"10px 18px", borderRadius:"7px", cursor:"pointer", fontSize:"13px",
              border: dark ? "1px solid #1e2a3a" : "1px solid #cbd5e1",
              color: dark ? "#789" : "#64748b",
            }}>{t.cancel}</button>
          </div>
        </div>
...
const i18n = {
  ko: {
    ...
    statMachines: "등록 기종", statMakers: "글로벌 제조사", statSpecs: "비교 스펙 항목",
  ...
  en: {
    ...
    statMachines: "Models", statMakers: "Global Makers", statSpecs: "Spec Items",
...
export default function App({ machines: externalMachines }) {
  ...
  const Stars = ({ rating }) => (
    <span style={{ color:"#fbbf24", fontSize:"12px" }}>
      ...
      <span style={{ color: dark?"#666":"#94a3b8", marginLeft:"4px", fontSize:"11px" }}>{rating}</span>
    </span>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", backgroundColor:c.bg, color:c.text, fontFamily:"'Noto Sans KR','IBM Plex Sans',sans-serif", position:"relative", overflowX:"hidden", transition:"background 0.3s, color 0.3s" }}>
      ...
      <header style={{ position:"sticky", top:0, zIndex:100, background:c.headerBg, backdropFilter:"blur(12px)", borderBottom:`1px solid ${c.headerBorder}`, transition:"background 0.3s" }}>
        <div style={{ maxWidth:"1200px", margin:"0 auto", padding:"12px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
            ...
            <div>
              <div style={{ fontSize:"17px", fontWeight:"800", letterSpacing:"3px", color:"#4fc3f7", fontFamily:"'IBM Plex Mono',monospace" }}>{t.siteTitle}</div>
              <div style={{ fontSize:"10px", color:c.textMuted, letterSpacing:"1px" }}>{t.siteSub}</div>
            </div>
          </div>
          <nav style={{ display:"flex", gap:"8px", alignItems:"center" }}>
            <button onClick={() => setView("browse")} style={{ background:"transparent", border:`1px solid ${view==="browse"?"#4fc3f7":c.pillBorder}`, color:view==="browse"?"#4fc3f7":c.pillColor, padding:"8px 18px", borderRadius:"6px", cursor:"pointer", fontSize:"13px", background:view==="browse"?"rgba(79,195,247,0.08)":"transparent" }}>{t.navBrowse}</button>
      ...
      <main style={{ maxWidth:"1200px", margin:"0 auto", padding:"0 24px 60px", position:"relative", zIndex:1 }}>
        ...
        {view === "browse" && (<>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:"40px" }}>
              {[[machineData.length+"+", t.statMachines],["6", t.statMakers],["22+", t.statSpecs]].map(([n,l]) => (
                <div key={l} style={{ display:"flex", flexDirection:"column", alignItems:"center" }}>
                  <span style={{ fontSize:"26px", fontWeight:"800", color:"#4fc3f7", fontFamily:"'IBM Plex Mono',monospace" }}>{n}</span>
                  <span style={{ fontSize:"11px", color:c.textMuted, marginTop:"2px" }}>{l}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          {/* Filters */}
          <section style={{ display:"flex", flexWrap:"wrap", gap:"16px", marginBottom:"20px", padding:"18px", background:c.filterBg, borderRadius:"12px", border:`1px solid ${c.filterBorder}`, transition:"background 0.3s" }}>
            {[{label:t.filterType, list:types, state:filterType, set:setFilterType}, {label:t.filterMaker, list:makers, state:filterMaker, set:setFilterMaker}].map(({label,list,state,set}) => (
              <div key={label} style={{ display:"flex", alignItems:"center", gap:"10px", flexWrap:"wrap" }}>
                <span style={{ fontSize:"11px", color:c.textMuted, letterSpacing:"1px", minWidth:"50px" }}>{label}</span>
                <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
                  {list.map((item) => (
                    <button key={item} onClick={() => set(item)} style={{
                      background: state===item ? "rgba(79,195,247,0.1)" : "transparent",
                      border: `1px solid ${state===item ? "#4fc3f7" : c.pillBorder}`,
                      color: state===item ? "#4fc3f7" : c.pillColor,
            ...
            {filtered.map((machine, i) => (
              <div key={machine.id}
                ...
                <div style={{ padding:"0 18px 6px" }}>
                  ...
                  <span style={{ color:c.textMuted, fontSize:"11px", marginLeft:"6px" }}>({machine.reviews} {t.reviews})</span>
                </div>
                  ...
                  {previewKeys.map((k) => (
                    <div key={k} style={{ display:"flex", justifyContent:"space-between", fontSize:"11.5px", borderBottom:`1px solid ${c.specItemBorder}`, paddingBottom:"4px" }}>
                      ...
                      <span style={{ color:c.specValColor, fontFamily:"'IBM Plex Mono',monospace", fontSize:"11px", textAlign:"right", maxWidth:"55%" }}>
                        {machine.specs?.[k] ?? machine[k==="최대 소재 크기"?"max_workpiece_size":k==="최대 급이송속도"?"max_rapid_feed":k] ?? "–"}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))" }}>
                  {group.keys.map((key) => (
                    <div key={key} style={{ padding:"12px 20px", borderRight:`1px solid ${c.specItemBorder}`, borderBottom:`1px solid ${c.specItemBorder}`, background:c.cardBg }}>
                      ...
                      <div style={{ fontSize:"13px", color:c.specValColor, fontFamily:"'IBM Plex Mono',monospace", fontWeight:"600" }}>{detailMachine.specs?.[key]??"–"}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <footer style={{ borderTop:`1px solid ${c.specBlockBorder}`, padding:"22px", position:"relative", zIndex:1 }}>
        <div style={{ maxWidth:"1200px", margin:"0 auto", display:"flex", gap:"12px", justifyContent:"center", fontSize:"12px" }}>
          <span style={{ color:c.footerColor }}>© 2025 MACHINEBASE</span>
          <span style={{ color:c.textMuted }}>·</span>
          <span style={{ color:c.footerColor }}>{t.footerText}</span>
        </div>
      </footer>
    </div>
  );
