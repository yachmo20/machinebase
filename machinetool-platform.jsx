...
export default function App({ machines: externalMachines, todayViews: initialTodayViews }) {
  ...
  // 히어로 통계 항목
  const heroStats = [
    [machineData.length+"+", t.statMachines],
    [new Set(machineData.map(m=>m.maker)).size.toString(), t.statMakers],
    ["22+", t.statSpecs],
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:"32px", flexWrap:"wrap" }}>
              {heroStats.map(([n, l]) => (
                <div key={l} style={{ display:"flex", flexDirection:"column", alignItems:"center" }}>
                  ...
                  <span style={{ fontSize:"11px", color:c.textMuted, marginTop:"2px" }}>
                    {l===t.statToday ? "👁 "+l : l}
                  </span>
                </div>
              ))}
            ...
            {[{label:t.filterType, list:types, state:filterType, set:setFilterType}, {label:t.filterMaker, list:makers, state:filterMaker, set:setFilterMaker}].map(({label,list,state,set}) => (
              <div key={label} style={{ display:"flex", alignItems:"center", gap:"10px", flexWrap:"wrap" }}>
                ...
                <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
                  {list.map((item) => (
                    <button key={item} onClick={() => set(item)} style={{ background: state===item?"rgba(79,195,247,0.1)":"transparent", border:`1px solid ${state===item?"#4fc3f7":c.pillBorder}`, color: state===item?"#4fc3f7":c.pillColor, padding:"5px 12px", borderRadius:"20px", cursor:"pointer", fontSize:"12px" }}>{getType(item)}</button>
                  ))}
                </div>
              </div>
            ))}
